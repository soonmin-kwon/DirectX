...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hWnd, LPCWSTR vsName, LPCWSTR pxName) {
	HRESULT result;
	...
	result = D3DCompileFromFile(vsName, NULL, NULL, "ColorVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(result)) {
		if (errorMessage) {
	...
	result = D3DCompileFromFile(pxName, NULL, NULL, "ColorPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result)) {
		if (errorMessage) {
			OutputShaderErrorMessage(errorMessage, hWnd, pxName);
		}
		else {
			MessageBox(hWnd, pxName, L"Missing Shader File", MB_OK);
		}
